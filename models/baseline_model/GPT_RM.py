...
class GPTJForRewardComparison(nn.Module):
    ...
    def forward(
            self, input_ids, attention_masks, token_type_ids,
            use_cache=False
            ):
        # Extract Data
        gcombo,bcombo = input_ids
        gattmasks,battmasks= attention_masks
        gtypeids,btypeids= token_type_ids
        ...
        logits = torch.cat([gscore.squeeze(-1), bscore.squeeze(-1)])
